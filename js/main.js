...
function initShopFilters() {
  ...
  function apply() {
    ...
    if (title) title.textContent = lloc ? `Encara no ens trobaràs a ${lloc}` : 'Encara no ens hi trobaràs';
...
function initBarriPrefill() {
  ...
  if (title) title.textContent = `Quina botiga de ${zona} hi encaixaria?`;
  if (lead) lead.textContent = `Encara no ens trobes a ${zona}, i ens hi agradaria arribar. Digues-nos quina botiga hi encaixaria i hi anirem a trucar de part teva.`;
}
...
function initForms() {
  document.querySelectorAll('form[data-form]').forEach((form) => {
    ...
    function personalitzaGracies() {
      ...
      if (slot && lloc && lloc.trim()) slot.textContent = ` de ${lloc.trim()}`;
    }
    ...
    form.addEventListener('submit', (e) => {
      ...
      form.querySelectorAll('[required]').forEach((input) => {
        ...
        if (err) err.textContent = valid ? '' : 'Aquest camp és obligatori';
        if (!valid) ok = false;
      });
      // Email
      ...
      if (email && email.value && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email.value)) {
        ...
        if (err) err.textContent = 'Introdueix un correu vàlid';
      }
      if (!ok) return;
      envia();
    });
